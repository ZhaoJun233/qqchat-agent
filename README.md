...
- **联网搜索**：模型在 JSON 里填 `search`（要查什么）或 `read`（要读哪个网页）时，机器人真的去查/去读，把结果作为**事实**交给下一轮 —— 而不是靠印象编。优先走**模型自带搜索**（检索在模型服务商那边完成、结果自带来源），不可用时回退到可插拔的搜索源模板（SearxNG / MediaWiki / 通用 HTML）；同一会话 30 秒内只查一次，搜不到/读不到会如实说“没查到”
